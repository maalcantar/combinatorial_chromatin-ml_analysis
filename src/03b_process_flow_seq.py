...
"""
Created on November 14 14:34:11 2023
@author: alcantar
example run 1: python 03b_process_flow_seq.py -i ../../combicr_data/combicr_outputs/maa-005_nozfbs/pickle_files/combiCR_results.pkl -m ../../combicr_data/Quintata_NGS_raw/MAA_005_noZFBS/AVITI_2023-10-23_noZFBS/Fastq/MAA_005_nozfbs_metadata.txt -r pMAA112_off pMAA112_on -t 100 -b 6
"""
...
def flow_seq_data_processing(reporter_id,
    ...
    --------------------
    reporter_id: str
        plasmid used to genomically intergrate the reporter [pMAA166, pMAA167]
    ...
    # only 6 bins because there were essentially no cells in bins 7 and 8
    # usually ~10 cells in those bins
    pMAA06_112_off_rep1_cell_counts = [300341, 800000,800000,
                                       800000, 800000, 10010]
    pMAA06_112_on_rep1_cell_counts = [309071, 800000,800000,
                                       800000, 800000, 10013]

    pMAA06_112_off_rep2_cell_counts = [309247, 800000,800000,
                                       800000, 800000, 10009]
    pMAA06_112_on_rep2_cell_counts = [202895, 800000,800000,
                                       800000, 800000, 10016]

    # fraction of cells from each sorting bin (per replicate)
    pMAA06_112_off_rep1_cell_fracs = [cell_count_bin/sum(pMAA06_112_off_rep1_cell_counts) for cell_count_bin in pMAA06_112_off_rep1_cell_counts]
    pMAA06_112_on_rep1_cell_fracs = [cell_count_bin/sum(pMAA06_112_on_rep1_cell_counts) for cell_count_bin in pMAA06_112_on_rep1_cell_counts]
    pMAA06_112_off_rep2_cell_fracs = [cell_count_bin/sum(pMAA06_112_off_rep2_cell_counts) for cell_count_bin in pMAA06_112_off_rep2_cell_counts]
    pMAA06_112_on_rep2_cell_fracs = [cell_count_bin/sum(pMAA06_112_on_rep2_cell_counts) for cell_count_bin in pMAA06_112_on_rep2_cell_counts]

    # sequencing counts for each bin
    # bins 7 and 8 had essentially no cells (between 1 and 14). so those bins
    # were not sequenced
    pMAA06_112_off_rep1_seq_counts = [2113068, 2508474, 2588029,
                                      1778171, 2561684, 2192291]
    pMAA06_112_on_rep1_seq_counts = [2114246, 1827375, 2225107,
                                     1727573, 1936961, 1927319]

    pMAA06_112_off_rep2_seq_counts = [2630862, 2268801, 2802564,
                                      2487254, 2858206, 2626178]
    pMAA06_112_on_rep2_seq_counts = [2304216, 1591270, 2313412,
                                     2040975, 2390656, 3127561]

    # dictionaries with cell fraction and read count information
    cell_fracs_dict = {'pMAA112_off_rep_1': pMAA06_112_off_rep1_cell_fracs,
                        'pMAA112_on_rep_1': pMAA06_112_on_rep1_cell_fracs,
                        'pMAA112_off_rep_2': pMAA06_112_off_rep2_cell_fracs,
                        'pMAA112_on_rep_2': pMAA06_112_on_rep2_cell_fracs}

    counts_dict = {'pMAA112_off_rep_1': pMAA06_112_off_rep1_seq_counts,
                    'pMAA112_on_rep_1': pMAA06_112_on_rep1_seq_counts,
                    'pMAA112_off_rep_2': pMAA06_112_off_rep2_seq_counts,
                    'pMAA112_on_rep_2': pMAA06_112_on_rep2_seq_counts}

    # extract samples with reporter_id (pMAA166 or pMAA167) of interest and
    ...
    for replicate in replicate_ids_list:
        ...
        if num_bins==4: # not implemented with updated normalization methods
           ...
           flow_seq_score_col_name = 'flow_seq_score'+'_rep_'+str(replicate)
           # calculate normalized flow seq score by taking weighted average
           # take into account that almost no cells (i.e., between 1 and 14)
           # were found in bins 7 and 8. Those bins were not collected or sent for
           # sequencing, but we take them into account in the normalization
           flow_seq_results_master_normed_df[flow_seq_score_col_name] = (flow_seq_results_master_normed_df[bin_names[0]]*0 \
                                                                      + flow_seq_results_master_normed_df[bin_names[1]]*(1/7) \
                                                                      + flow_seq_results_master_normed_df[bin_names[2]]*(2/7) \
                                                                      + flow_seq_results_master_normed_df[bin_names[3]]*(3/7) \
            ...
            print('Flow-seq experiments should have either 4,6,or 8 bins. Aborting function.')
            return
