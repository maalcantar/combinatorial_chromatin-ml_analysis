...
"""
Created on January 16 22:35:51 2023
@author: alcantar
example run: python extract_protein_physiochemical_features.py
"""
...
def cal_discriptors(secq):
    ...
    '''calculates physiochemical features from an amino acid sequence. this
     function was obtained from Huang and Xu et al (2023)
    ...
    returns
    ---------
    pep_discriptor: dictionary
        dictionary with physiochemical descriptors
    '''
...
def main():
    ...
    # load in dictionary with protein name-to-sequence relationships
    protein_name_to_aa_sequence_dict_path = '../../data/combicr_protien_to_aa_sequence_dict.pkl'

    with open(protein_name_to_aa_sequence_dict_path, 'rb') as handle:
        protein_name_to_aa_sequence_dict = pickle.load(handle)

    # calculate physiochemical feature for all proteins
    protein_to_physiochemical_feats_dict = dict()
    for protein in protein_name_to_aa_sequence_dict:
        protein_aa_sequence = protein_name_to_aa_sequence_dict[protein]
        protein_descriptors_temp = cal_discriptors(protein_aa_sequence)
        protein_to_physiochemical_feats_dict.update({protein: protein_descriptors_temp})

    # manually define features for empty regulator
    protein_to_physiochemical_feats_dict.update({'empty': {'Mw': 0.0,
                                                         'charge of all': 0,
                                                         'positive_charge':0,
                                                         'negative_charge':0,
                                                         'polar_number':0,
    ...
    # save features to pickle file (as dictionary)
    protein_to_physiochemical_feats_dict_path = \
    '../../data/physiochemical_features_raw/combicr_physiochemical_features_dict.pkl'

    with open(protein_to_physiochemical_feats_dict_path, 'wb') as handle:
             pickle.dump(protein_to_physiochemical_feats_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
